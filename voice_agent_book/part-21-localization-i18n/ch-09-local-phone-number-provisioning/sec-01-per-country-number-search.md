# Per-Country Number Search

## Overview

This section covers Per-Country Number Search in depth. The capability is part of the 09 Local Phone Number Provisioning chapter and provides critical functionality for the voice agent platform's production operations.
...
kind: Deployment
metadata:
  name: per-country-number-search
spec:
  replicas: 3
  strategy:
    type: RollingUpdate
  ...
  template:
    spec:
      containers:
        - name: service
          image: voiceagent/per-country-number-search:latest
          ports:
            - containerPort: 3000
          env:
            - name: NODE_ENV
...
Per-Country Number Search is designed for production reliability from day one. The patterns and practices described here ensure the capability meets the high standards required for enterprise voice AI deployments.
