# Section 06: DTMF Collection & Buffering

## Overview

DTMF Collection & Buffering is a critical component within the DTMF Detection & Keypad Handling system of the 04 - Core Voice Engine module. This section provides a comprehensive exploration of the architecture, design decisions, implementation patterns, and production considerations required to build this component in a production AI voice agent platform.
...
The implementation covers DTMF Detection & Keypad Handling with a focus on dtmf collection & buffering. Key concerns include real-time performance, fault tolerance, observability, and security.

## Architecture

```
+----------+    +----------+    +----------+    +----------+    +----------+
| RTP      |--->| Goertzel |--->| Digit    |--->| Buffer   |--->| Menu     |
| Audio    |    | Algorith |    | Decode   |    | (interdi |    | Router   |
| Stream   |    | m (freq) |    | (0-9,*,#)|    |  git)    |    | (IVR)    |
+----------+    +----------+    +----------+    +----------+    +----------+
```
...
- **RFC 2833 Preferred**: Out-of-band DTMF in RTP header extension. More reliable than in-band.
- **In-Band Fallback**: Goertzel algorithm detect DTMF tones in audio. For legacy SIP trunk compatibility.
- **Interdigit Timeout**: Configurable timeout between digits (default: 3s). Complete on # or timeout.
## Implementation Approach

```typescript
interface DTMFCollectionAndBufferingConfig {
  enabled: boolean;
...
class DTMFCollectionAndBufferingComponent {
  private config: DTMFCollectionAndBufferingConfig;
  private metrics: MetricsCollector;
  
  constructor(config: DTMFCollectionAndBufferingConfig) {
    this.config = config;
    this.metrics = new MetricsCollector('dtmf_collection_&_buffering');
  }
...
- **FreeSWITCH** (MPL 2.0): DTMF
- **SIP.js** (MIT): RFC 2833
- **Goertzel alg** (Public): Tone detect
