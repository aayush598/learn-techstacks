...
This section covers Terraform/Pulumi Setup in depth. The capability is part of the 05 Infrastructure As Code chapter and provides critical functionality for the voice agent platform's production operations.
...
spec:
  ...
  template:
    spec:
      containers:
        - name: service
          image: voiceagent/terraform/pulumi-setup:latest
          ports:
            - containerPort: 3000
          env:
            - name: NODE_ENV
...
Terraform/Pulumi Setup is designed for production reliability from day one. The patterns and practices described here ensure the capability meets the high standards required for enterprise voice AI deployments.
