# Section: Archival Lifecycle Policies

Archival Lifecycle Policies is a core component of the secure storage and archival system. This section examines its architecture, implementation, and operational considerations.
...
**Policy conflict resolution.** When multiple lifecycle policies apply to the same recording, the most restrictive policy wins. For retention duration, the longest retention period is applied. For deletion actions, the latest deletion date is used. Policy conflicts are logged and flagged for administrator review.
