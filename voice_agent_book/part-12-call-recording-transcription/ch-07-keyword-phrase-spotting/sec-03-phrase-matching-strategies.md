# Section: Phrase Matching Strategies

Phrase Matching Strategies is a core component of the keyword and phrase spotting system. This section examines its architecture, implementation, and operational considerations.
...
**Aho-Corasick automaton.** The keyword matcher uses the Aho-Corasick string-matching algorithm for O(n) multi-pattern matching. The keyword catalog is compiled into a finite state machine at load time, enabling real-time matching against thousands of patterns simultaneously. The automaton supports wildcard prefixes and suffixes, case-insensitive matching, and proximity-aware matching where multiple keywords within a configurable word window trigger a hit.
## 
## Pseudo-code
...
**Multi-strategy matching pipeline.** The phrase matching pipeline applies strategies in order: exact matching first (fastest), followed by case-insensitive matching, fuzzy matching with configurable edit distance, and finally regex matching (most flexible but slowest). This tiered approach optimizes for throughput.
