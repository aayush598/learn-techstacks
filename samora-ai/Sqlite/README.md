...
**Code:**
```sql
-- SQLite is embedded: one self-contained database file, no server
.open example.db
...
```python
import sqlite3
con = sqlite3.connect('app.db')   # no host/port/user/password to configure
...
CREATE TABLE t (
    ...
    score  REAL,      -- REAL (floating point)
    data   BLOB,      -- BLOB (binary)
    active INTEGER    -- bool stored as 0/1
);
INSERT INTO t VALUES (1, 'hi', 3.14, X'DEAD', 1);
SELECT typeof(id), typeof(name), typeof(score), typeof(data) FROM t;
```
...
**Code:**
```sql
CREATE TABLE t(v INTEGER);               -- INTEGER affinity
INSERT INTO t VALUES (42);
INSERT INTO t VALUES ('text');           -- affinity NOT enforced
SELECT v, typeof(v) FROM t;              -- 42|integer, text|text
```
...
SELECT id FROM log;                      -- 2: deleted rowid is never reused
```
...
PRAGMA max_page_count;                   -- ~2^32-2 pages -> ~140 TB at 4096
```
...
**Code:**
```sql
PRAGMA journal_mode = WAL;               -- readers proceed while writer appends to -wal file
...
**Code:**
```sql
PRAGMA journal_mode = DELETE;            -- rollback journal deleted on commit
PRAGMA journal_mode = TRUNCATE;          -- journal truncated, not deleted
PRAGMA journal_mode = PERSIST;           -- journal overwritten in place
PRAGMA journal_mode = MEMORY;            -- journal kept in memory
PRAGMA journal_mode = WAL;               -- write-ahead log
PRAGMA journal_mode = OFF;               -- no journal at all
```
...
con.execute("BEGIN DEFERRED")            # lock acquired on first read/write
con.execute("INSERT INTO t VALUES (1)")
con.commit()
con.execute("BEGIN IMMEDIATE")           # reserved lock taken immediately
con.execute("INSERT INTO t VALUES (2)")
con.commit()
con.execute("BEGIN EXCLUSIVE")           # exclusive lock, blocks readers too
con.commit()
```
...
```python
import sqlite3
a = sqlite3.connect('t.db', timeout=0)
b = sqlite3.connect('t.db', timeout=0)
a.execute("CREATE TABLE IF NOT EXISTS t(x)")
a.execute("BEGIN EXCLUSIVE")             # one writer only
try:
    b.execute("UPDATE t SET x = 1")      # SQLITE_BUSY
except sqlite3.OperationalError:
    print("SQLITE_BUSY - single writer")
a.commit()
```
...
**Code:**
```sql
PRAGMA journal_mode = WAL;               -- writes go to -wal, readers unaffected
PRAGMA journal_mode = DELETE;            -- rollback journal; readers blocked
```
...
PRAGMA freelist_count;                   -- free pages still in the file
VACUUM;                                  -- rebuilds file and returns pages to OS
```
...
**Code:**
```sql
PRAGMA foreign_keys = ON;                -- per connection, not stored in file
CREATE TABLE parent(id INTEGER PRIMARY KEY);
CREATE TABLE child(pid INTEGER REFERENCES parent(id));
INSERT INTO child VALUES (999);          -- FOREIGN KEY constraint failed
```
...
WITH RECURSIVE seq(n) AS (
    ...
    UNION ALL
    SELECT n + 1 FROM seq WHERE n < 10   -- recursive term
)
SELECT n FROM seq;
```
...
**Code:**
```sql
CREATE INDEX idx_users_name ON users(name);                    -- B-tree
CREATE UNIQUE INDEX idx_users_email ON users(email);           -- UNIQUE
CREATE INDEX idx_active ON users(created_at) WHERE active = 1; -- partial
CREATE INDEX idx_lower ON users(LOWER(name));                  -- expression
CREATE INDEX idx_desc ON orders(total DESC);                   -- descending
```
...
**Code:**
```sql
CREATE INDEX idx_pending ON jobs(id) WHERE status = 'pending';
SELECT * FROM jobs WHERE status = 'pending' ORDER BY id;   -- uses idx_pending
EXPLAIN QUERY PLAN SELECT status, id FROM jobs WHERE status='pending';
```
...
SELECT * FROM users WHERE LOWER(name) = 'john';   -- index seek, not full scan
```
...
INSERT INTO docs VALUES ('SQLite', 'embedded full-text search');
...
**Code:**
```sql
CREATE VIRTUAL TABLE emails USING fts5(sender, subject, body);
INSERT INTO emails VALUES ('alice@x.com', 'Hello', 'body text');
SELECT sender, subject FROM emails WHERE emails MATCH 'Hello';
```
...
**Code:**
```sql
CREATE VIRTUAL TABLE d5 USING fts5(content);    -- current, recommended
CREATE VIRTUAL TABLE d4 USING fts4(content);    -- legacy
CREATE VIRTUAL TABLE d3 USING fts3(content);    -- oldest
```
...
**Code:**
```sql
-- generate_series virtual table fabricates rows on demand
SELECT value FROM generate_series(1, 5);
-- dbstat and pragma_table_info are also virtual tables
...
**Code:**
```sql
SELECT * FROM users WHERE name LIKE 'a%';    -- % = any sequence, case-insensitive
SELECT * FROM users WHERE name LIKE '_ob';   -- _ = one character
PRAGMA case_sensitive_like = ON;             -- make LIKE case-sensitive
SELECT 'A' LIKE 'a';                         -- 0 afterwards
```
...
**Code:**
```sql
SELECT 'abc' GLOB 'a*';          -- 1: * matches any sequence
SELECT 'abc' GLOB 'a?c';         -- 1: ? matches one character
SELECT 'Abc' GLOB 'a*';          -- 0: GLOB is always case-sensitive
SELECT 'bbc' GLOB '[a-c]bc';     -- 1: character range
```
...
SELECT * FROM t WHERE email IS NULL;         -- correct NULL test
SELECT * FROM t WHERE email = NULL;          -- never matches
INSERT INTO t(email) VALUES (NULL);          -- UNIQUE allows multiple NULLs
...
**Code:**
```sql
SELECT name FROM a UNION SELECT name FROM b;       -- deduplicates (sorts)
SELECT name FROM a UNION ALL SELECT name FROM b;   -- keeps duplicates, faster
```
...
**Code:**
```sql
CREATE TABLE logs(ts TEXT, msg TEXT);
CREATE TRIGGER log_insert AFTER INSERT ON users
BEGIN
    INSERT INTO logs(ts, msg) VALUES (datetime('now'), 'user added');
END;
INSERT INTO users(name) VALUES ('Bob');        -- fires the trigger
SELECT * FROM logs;
```
...
**Code:**
```sql
-- no row-count ceiling in SQLite; only disk space bounds the table
CREATE TABLE t(id INTEGER PRIMARY KEY);
SELECT MAX(id) FROM t;
PRAGMA max_page_count;                         -- pages, not an explicit row cap
```
...
SELECT count(*) FROM pragma_table_info('wide');   -- 5 columns
-- hard ceiling is 32767; practical limit ~2000 due to the 1 GB row size
```
...
-- recreate pattern to change a column definition:
CREATE TABLE users_new(id INTEGER PRIMARY KEY, name TEXT NOT NULL);
INSERT INTO users_new SELECT id, name FROM members;
DROP TABLE members;
ALTER TABLE users_new RENAME TO members;
```
...
**Code:**
```sql
-- sqlite3 CLI is zero-config; it opens a db file directly
sqlite3 app.db
.tables
.schema users
.headers on
...
SELECT * FROM users;
.output stdout
.dump                     -- full database as SQL (portable backup)
.backup backup.db         -- binary copy
```
...
**Code:**
```sql
CREATE TABLE a(id INTEGER PRIMARY KEY, v TEXT);  -- id aliases rowid
CREATE TABLE b(id INT PRIMARY KEY, v TEXT);      -- INT is NOT the rowid alias
INSERT INTO a(v) VALUES ('x');
INSERT INTO a(v) VALUES ('y');
SELECT id FROM a;              -- 1, 2 auto-assigned from rowid
INSERT INTO b(v) VALUES ('x'); -- error: b.id has no default value
```
...
SELECT rowid, _rowid_, oid, name FROM t;   -- all three names work
```
...
SELECT rowid FROM t;   -- error: no rowid column exists
```
...
**Code:**
```sql
PRAGMA synchronous = OFF;     -- 0: no syncing, fastest, risks corruption on crash
PRAGMA synchronous = NORMAL;  -- 1: sync at critical moments (good with WAL)
PRAGMA synchronous = FULL;    -- 2: sync every commit, safest
PRAGMA synchronous = EXTRA;   -- 3: extra sync in WAL mode
```
...
-- RIGHT / FULL OUTER JOIN are simulated with LEFT JOIN + UNION
```
...
```sql
-- scalar subquery
SELECT id, (SELECT MAX(total) FROM orders) AS max_total FROM orders;
-- derived table
...
**Code:**
```sql
-- IN: subquery results are materialized into a list
SELECT * FROM users WHERE id IN (SELECT user_id FROM orders);
-- EXISTS: short-circuits on the first match (better for correlated queries)
SELECT * FROM users u
WHERE EXISTS (SELECT 1 FROM orders o WHERE o.user_id = u.id);
```
...
SELECT * FROM t;   -- created and retries filled by their defaults
```
...
INSERT INTO person(age) VALUES (-1);   -- CHECK constraint failed: age >= 0
```
...
```sql
CREATE TABLE t (
    id       INTEGER PRIMARY KEY,   -- one per table; NOT NULL implied
    email    TEXT UNIQUE,           -- several UNIQUE columns allowed
    username TEXT UNIQUE
);
INSERT INTO t(email, username) VALUES (NULL, 'a');
INSERT INTO t(email, username) VALUES (NULL, 'b');   -- both NULLs accepted
```
...
INSERT OR REPLACE INTO t VALUES (1, 'new');  -- deletes row 1, inserts new row
SELECT * FROM t;                            -- 1|new
```
...
INSERT OR IGNORE INTO t VALUES (1, 'b');    -- silently ignored
...
```sql
BEGIN;
INSERT OR ROLLBACK INTO t VALUES (1, 'a');
INSERT OR ROLLBACK INTO t VALUES (1, 'dup');  -- conflict -> whole txn undone
COMMIT;
SELECT COUNT(*) FROM t;                    -- 0: everything was rolled back
```
...
```python
import sqlite3
r1 = sqlite3.connect('app.db')              # readers can share the file
r2 = sqlite3.connect('app.db')
w  = sqlite3.connect('app.db')
w.execute("PRAGMA journal_mode = WAL")      # readers + one writer coexist
w.execute("PRAGMA busy_timeout = 5000")     # writers still serialize
```
...
```python
import sqlite3
# shared cache: two connections in the same process share one cache
...
```python
import sqlite3
w = sqlite3.connect('t.db', timeout=0)      # 0 ms: fail immediately on lock
r = sqlite3.connect('t.db', timeout=0)
w.execute("CREATE TABLE IF NOT EXISTS t(x)")
w.execute("BEGIN EXCLUSIVE")
w.execute("INSERT INTO t VALUES (1)")
try:
    r.execute("INSERT INTO t VALUES (2)")   # SQLITE_BUSY: database is locked
...
```python
import sqlite3
con = sqlite3.connect('t.db', timeout=5000) # wait up to 5s before SQLITE_BUSY
con.execute("PRAGMA busy_timeout = 5000")    # explicit equivalent
```
...
SELECT * FROM other.t;      -- error: 1st parameter to ATTACH has been detached
```
...
**Code:**
```sql
-- SQLite has no built-in encryption; use SQLCipher / SEE builds
PRAGMA key = 'passphrase';       -- SQLCipher: encrypts pages at rest
CREATE TABLE secrets(v TEXT);    -- written as AES-256 encrypted pages
```
...
**Code:**
```sql
sqlcipher app.db                 -- open the encrypted file with the SQLCipher CLI
PRAGMA key = 'secret';
SELECT * FROM users;             -- transparent decryption at the page level
```
...
**Code:**
```sql
CREATE TABLE t(data TEXT, name TEXT GENERATED ALWAYS AS
    (json_extract(data, '$.name')) VIRTUAL);
INSERT INTO t(data) VALUES ('{"name":"John","age":30}');
SELECT name FROM t;                       -- John
CREATE INDEX idx_t_name ON t(name);       -- index a JSON field
```
...
SELECT '{"a":1}' ->  '$.a';                          -- 1 (JSON type)
SELECT '{"a":1}' ->> '$.a';                          -- 1 (plain SQL scalar)
```
...
-- key=0/1/2, value=10/20/30, type=integer
SELECT j.key, j.value FROM t, json_each(t.tags) j;   -- unnest array in a table
```
...
BEGIN;                             -- batch many writes into one transaction
INSERT INTO t(x) SELECT value FROM generate_series(1, 1000);
COMMIT;
```
...
**Code:**
```sql
PRAGMA page_size = 65536;   -- larger pages: fewer reads for big BLOBs/scans
PRAGMA page_size;           -- note: must be set before any table exists
-- small rows -> small pages (1024/4096) waste less space per page
```
...
**Code:**
```sql
PRAGMA mmap_size = 268435456;   -- 256 MB of the file memory-mapped
PRAGMA mmap_size;               -- 0 disables memory-mapped I/O
```
...
**Code:**
```sql
PRAGMA cache_size = -64000;   -- 64 MB cache (negative = kibibytes)
PRAGMA cache_size;            -- read back the current setting
```
...
**Code:**
```sql
PRAGMA temp_store = MEMORY;   -- 2: temp tables/indexes in RAM
PRAGMA temp_store = FILE;     -- 1: on disk
PRAGMA temp_store = DEFAULT;  -- 0: use compile-time default
```
...
live.backup(backup)      # uses sqlite3_backup_init/step/finish internally
backup.close()
```
...
**Code:**
```sql
.backup backup.db        # binary copy: fast, exact, feature-complete
.dump > dump.sql         # SQL text: portable, readable, slower to restore
```
...
**Code:**
```sql
-- BAD: every INSERT is its own implicit transaction
INSERT INTO t(x) VALUES (1);
INSERT INTO t(x) VALUES (2);
-- GOOD: one explicit transaction around the batch
BEGIN;
INSERT INTO t(x) VALUES (1);
INSERT INTO t(x) VALUES (2);
COMMIT;
...
SELECT GROUP_CONCAT(name) FROM users;          -- a,b,c,d
SELECT GROUP_CONCAT(name, ' | ') FROM users;   -- custom separator
```
...
SELECT * FROM users WHERE id > 20 ORDER BY id LIMIT 10;    -- keyset (cursor)
```
...
**Code:**
```sql
-- SQLite uses standard LIMIT ... OFFSET; there is no TOP
SELECT * FROM users LIMIT 5;    -- TOP 5 is SQL Server syntax, not SQLite
```
...
**Code:**
```sql
SELECT PRINTF('Hello, %s!', name) FROM users;
SELECT PRINTF('%04d-%02d', 2026, 9);   -- 2026-09
```
...
CREATE INDEX idx_area ON t(area);       -- index a derived, non-stored value
SELECT * FROM t WHERE area > 10;
```
...
INSERT INTO t(width, height) VALUES (2, 3);
SELECT area FROM t;                     -- 6.0, read back without recomputing
```
...
**Code:**
```sql
CREATE TABLE prices(new_price REAL, product_id INTEGER);
INSERT INTO products_prices VALUES (9.99, 5);
UPDATE products p
...
**Code:**
```sql
SELECT 'A' = 'a';            -- 0: UPPER()/LOWER() only fold ASCII A-Z
SELECT UPPER('café');        -- CAFé : the é is left untouched
SELECT 'Ä' = 'ä';            -- 0 unless an ICU/custom collation is used
```
...
**Code:**
```sql
-- ICU extension (built with -DSQLITE_ENABLE_ICU)
SELECT 'á' < 'b';                     -- 0: BINARY = byte-wise
SELECT 'á' < 'b' COLLATE ICU;         -- 1: Unicode collation
SELECT * FROM users ORDER BY name COLLATE ICU;   -- locale-aware sort
```
...
**Code:**
```sql
-- default max TEXT/BLOB length is 1e9 bytes (SQLITE_MAX_LENGTH)
SELECT length(hex(randomblob(100)));     -- 200 bytes of TEXT
SELECT x'FF' FROM t WHERE ...;           -- exceeding the limit -> SQLITE_TOOBIG
```
...
**Code:**
```sql
-- default: 999 bound parameters per prepared statement
SELECT * FROM t WHERE id IN (?, ?, ?);
-- pass big lists as a JSON array instead:
...
import sqlite3
con = sqlite3.connect(':memory:')
# sqlite3_exec(): compile and run zero or more statements in one call
con.executescript("CREATE TABLE t(x); INSERT INTO t VALUES (1),(2);")
print(con.execute("SELECT COUNT(*) FROM t").fetchone()[0])   # 2
```
...
stmt = con.execute("INSERT INTO t VALUES (?)")   # prepared once
for i in range(100):
    stmt.execute((i,))                          # bind + step repeatedly
con.commit()
```
...
import sqlite3
con = sqlite3.connect(':memory:')
name = "alice'; DROP TABLE users;--"            # untrusted user input
# parameter binding neutralizes the quote/injection
...
print(con.total_changes)                         # 3 so far
print(con.execute("SELECT changes()").fetchone()[0])   # last statement's count
con.execute("UPDATE t SET x = x + 1 WHERE x > 1")
print(con.total_changes)                         # 5 (cumulative)
```
...
PRAGMA compile_options;                          -- compile-time feature flags
```
...
**Code:**
```sql
-- SQLite: transactional, SQL, single file, embedded (OLTP)
BEGIN;
INSERT INTO kv(k, v) VALUES ('a', x'01');
COMMIT;                                          -- atomic vs LSM key-value stores
```
...
**Code:**
```sql
PRAGMA integrity_check;   -- ok when the database is structurally sound
```
...
**Code:**
```sql
PRAGMA quick_check;       -- 'ok': faster subset of integrity_check
```
...
```python
import sqlite3
# crash safety: journal/WAL is replayed or rolled back on next open
con = sqlite3.connect('app.db')
con.execute("PRAGMA journal_mode = WAL")
con.execute("BEGIN IMMEDIATE")
con.execute("INSERT INTO t VALUES (1)")
con.commit()                     # durable commit; crash recovery on reopen
```
