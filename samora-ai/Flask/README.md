...
def index():
    ...
    return f"You visited {session['visits']} times"
```
...
@app.route("/")
def index():
    # Flask ships with no ORM, admin or auth — bring them in yourself
    return "Flask micro-app; add extensions as needed"
```
...
def list_posts():
    ...
    return f"Showing page {page} -> {url_for('show_post', post_id=10)}"
```
...
# index.html: {% for name in names %}{{ name|upper }}{% endfor %}
```
...
def create_user():
    ...
    return jsonify({"created": True, "name": data["name"]})
```
...
def index():
    ...
    return f"Stylesheet at {url_for('static', filename='style.css')}"
```
...
def index():
    ...
    return f"{request.method} {request.path}, user={session['user']}"
```
...
@app.route("/")
def index():
    return get_db().execute("SELECT 1")
```
...
def set_cookie():
    ...
    resp.set_cookie("key", "value", max_age=3600, httponly=True)
    return resp

@app.route("/get")
def get_cookie():
    return request.cookies.get("key", "missing")
```
...
new_post = Post(title="Hello Flask")
...
@app.before_request
def hook():
    print("Hook: runs in request order")

def signal_handler(sender, **extra):
    print("Signal: decoupled subscriber")
...
@app.route("/send")
def send():
    msg = Message("Subject", recipients=["to@example.com"])
    msg.body = "Email body"
    mail.send(msg)
    return "Sent"
```
...
msg = Message("Hi", recipients=["a@x.com", "b@x.com"])
msg.html = "<b>HTML body</b>"
msg.attach("data.csv", "text/csv", "a,b\n1,2")
mail.send(msg)
```
...
with app.app_context():
    print(current_app.config["DEBUG"])  # uses proxy, not imported app
```
...
@app.route("/")
def index():
    return "Dev server with reloader + debugger"
...
class CustomJSONProvider(DefaultJSONProvider):
    def dumps(self, obj, **kwargs):
        return super().dumps(obj, sort_keys=True, **kwargs)
...
def posts():
    ...
    return jsonify(
        items=[p.title for p in pagination.items],
...
app = Flask(__name__)
app.config.from_pyfile("config.cfg")  # KEY = 'value'
app.config.from_object("module.ConfigClass")
...
app = Flask(__name__)
config_class = os.environ.get("FLASK_CONFIG", "DevelopmentConfig")
app.config.from_object(locals()[config_class])
```
...
app = Flask(__name__)
app.secret_key = secrets.token_hex(16)   # new random key per start
# or os.environ["SECRET_KEY"]
# or os.urandom(24)
```
...
app.config.from_object(
    {"development": DevelopmentConfig,
     ...
     "production": ProductionConfig}[os.environ.get("FLASK_ENV", "development")]
)
```
...
# Run with: gunicorn -w 4 -b 0.0.0.0:8000 production:app
```
...
local.user = "bob"
print(user)  # bob, proxy resolves lazily
```
...
def worker():
    ctx.data.append(threading.current_thread().name)
    print(ctx.data)

for _ in range(4):
    threading.Thread(target=worker).start()
# each thread sees only its own ctx.data
```
...
def ready():
    ...
    return (jsonify(status="ready" if db_ok else "degraded"),
...
@app.route("/report")
def report():
    return stream_template("report.html", rows=range(10000))
```
...
def resource():
    ...
    resp.add_etag()  # computes ETag from response data
    return resp
```
...
def initialize():
    app.config["MODEL"] = load_ml_model()
# Deprecated: use app.before_request + flag, or factory pattern in 2.3+
```
...
app.add_url_rule("/items/<int:item_id>", view_func=ItemView.as_view("item"))
```
...
path = safe_join(base, user_input)  # raises NotFound
# "/var/data/files/../etc/passwd" -> NotFound
```
...
def comment():
    ...
    return f"Safe output: {escape(raw)}"  # &lt;script&gt;...
```
...
def stream_logs():
    def generate():
        user = request.args.get("user")  # needs request context
        for line in fetch_logs(user):
            yield line
...
app = Flask(__name__)
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://user:pass@db/app"
