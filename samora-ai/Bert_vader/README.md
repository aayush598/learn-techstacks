...
print(type(model).__name__)  # BertModel
```
...
# BERT reads LEFT and RIGHT context at the same time (bidirectional)
ids = tok.encode("The bank will close the account.", add_special_tokens=True)
print(tok.tokenize("close"), tok.convert_ids_to_tokens(ids))
```
...
model = BertModel(config)  # encoder stack only (no decoder)
...
print(attention(Q, Q, Q).shape)  # (1, 5, 64) weighted sums of all words
```
...
# BERT adds a *learned* positional embedding to each token embedding
pos_emb = nn.Embedding(512, 768)   # max_len=512, hidden=768
positions = torch.arange(5)
print(pos_emb(positions).shape)    # (5, 768)
```
...
for ckpt in ["bert-base-uncased", "bert-large-uncased"]:
    m = BertModel.from_pretrained(ckpt)
    print(ckpt, m.config.num_hidden_layers, m.config.num_attention_heads,
          m.config.hidden_size, sum(p.numel() for p in m.parameters()))
```
...
from transformers import BertTokenizer
tok = BertTokenizer.from_pretrained("bert-base-uncased")
print(tok.tokenize("eating"))        # WordPiece: ['eat', '##ing'] (## = subword)
print(tok.tokenize("unhappiness"))   # ['un', '##happiness']
print(tok.tokenize("[CLS] what a great movie [SEP]"))  # special tokens kept
```
...
from transformers import BertTokenizer
tok = BertTokenizer.from_pretrained("bert-base-uncased")
ids = tok.encode("Great movie", add_special_tokens=True)
print(ids)                            # [101, 2307, 3185, 102]
print(tok.convert_ids_to_tokens(ids)) # ['[CLS]', 'great', 'movie', '[SEP]']
```
...
from transformers import BertTokenizer
tok = BertTokenizer.from_pretrained("bert-base-uncased")
ids = tok.encode("question?", "answer", add_special_tokens=True)
...
from transformers import BertTokenizer
tok = BertTokenizer.from_pretrained("bert-base-uncased")
enc = tok("sentenceA", "sentenceB", return_tensors="pt")
print(enc["token_type_ids"])  # 0 for segment A, 1 for segment B
```
...
m = BertForPreTraining.from_pretrained("bert-base-uncased")
tok = BertTokenizer.from_pretrained("bert-base-uncased")
out = m(**tok("The cat sat on the mat.", return_tensors="pt"))
print(out.prediction_logits.shape)       # MLM head over vocab
print(out.seq_relationship_logits.shape) # NSP head: is_next / not_next
```
...
batch = collator([tok("The quick brown fox jumps", return_tensors="pt")])
print(batch["labels"])  # -100 where not masked; mask id where the 15% fell
```
...
```python
import random
def mask_decision(orig_id, mask_id, vocab_size):
    r = random.random()
    if r < 0.8:
        return mask_id                          # 80% -> [MASK]
    if r < 0.9:
        return random.randint(0, vocab_size-1)  # 10% -> random token
    return orig_id                              # 10% -> unchanged
print(mask_decision(1, 103, 30522))
```
...
m = BertForNextSentencePrediction.from_pretrained("bert-base-uncased")
tok = BertTokenizer.from_pretrained("bert-base-uncased")
out = m(**tok("The sky is blue.", "It is raining.", return_tensors="pt"))
print(out.logits)  # 2 logits: is_next / not_next
```
...
train_ds = [{"input_ids": [101, 7592, 1010, 102], "labels": 1}]  # tiny example
trainer = Trainer(
    model=m,
    args=TrainingArguments("out", learning_rate=2e-5, num_train_epochs=3),
    train_dataset=train_ds,  # a labeled, tokenized dataset
)
trainer.train()
```
...
# Feature-based: freeze BERT and use its outputs as features
m = AutoModel.from_pretrained("bert-base-uncased")
tok = AutoTokenizer.from_pretrained("bert-base-uncased")
with torch.no_grad():
    emb = m(**tok("Great movie", return_tensors="pt")).last_hidden_state[:, 0]
print(emb.shape)  # (1, 768) -> feed to a separate classifier
```
...
print(ner("Barack Obama lives in New York."))
print(qa(question="Who?", context="BERT was made by Google in 2018."))
```
...
print(sst2[0])             # {'sentence': ..., 'label': 0/1}
print(metric.compute(predictions=[0, 1, 1], references=[0, 1, 0]))
```
...
from transformers import pipeline
qa = pipeline("question-answering", model="distilbert-base-uncased-distilled-squad")
print(qa(question="Who developed BERT?",
...
out = m(**tok("Great movie", return_tensors="pt"))
cls_vec = out.last_hidden_state[:, 0]              # [CLS] embedding
last4 = torch.cat(out.hidden_states[-4:], dim=-1)  # concat of last 4 layers
print(cls_vec.shape, last4.shape)
```
...
m = AutoModel.from_pretrained("bert-base-uncased")
with torch.no_grad():
    a = m(**tok("I sat by the river bank.", return_tensors="pt")).last_hidden_state[:, 4]
    b = m(**tok("I deposited money in the bank.", return_tensors="pt")).last_hidden_state[:, 6]
print(torch.cosine_similarity(a, b))  # < 1.0 : same word, different context
```
...
print(len(tok("word " * 1000, truncation=True)["input_ids"]))  # capped at 512
```
...
tok = AutoTokenizer.from_pretrained("bert-base-uncased")
# 1) 512-token limit
print(len(tok("w " * 600, truncation=True)["input_ids"]))
# 2) computationally expensive, 3) encoder-only, 4) weak on negation
m = BertModel.from_pretrained("bert-base-uncased")
print(sum(p.numel() for p in m.parameters()))  # 110M params
```
...
from transformers import AutoTokenizer
tok = AutoTokenizer.from_pretrained("bert-base-uncased")
def sliding_windows(text, window=512, stride=256):
    ids = tok(text, add_special_tokens=False)["input_ids"]
    return [ids[i:i+window] for i in range(0, len(ids), stride)]
chunks = sliding_windows("word " * 3000)
print(len(chunks), "chunks, each <= 512 tokens")
```
...
m = AutoModel.from_pretrained("roberta-base")
...
print(len(m.transformer.layer))  # 6 layers, a distilled student
```
...
print(len(m.encoder.albert_layers))  # 12 layers, all SHARING parameters
```
...
out = m(**tok("The chef cooked the meal.", return_tensors="pt"))
print(out.logits.shape)  # predicts replaced vs original for EVERY token
```
...
```python
from transformers import AutoModel
m = AutoModel.from_pretrained("SpanBERT/spanbert-base-cased")
print(type(m).__name__)  # span-masked (contiguous spans) encoder
```
...
e1 = m.encode("I love this movie")
e2 = m.encode("This film is terrible")
print(util.cos_sim(e1, e2).item())  # low similarity for opposites
```
...
P, R, F1 = score(["the cat sat on the mat"], ["a cat sat on a rug"], lang="en")
print(F1.item())  # BERT-embedding based similarity, not n-grams
```
...
logits = m(**tok("Great movie", return_tensors="pt")).logits
print(torch.softmax(logits, dim=-1))  # [CLS] -> linear -> softmax
```
...
tok = AutoTokenizer.from_pretrained("bert-base-uncased")
m = AutoModelForTokenClassification.from_pretrained("bert-base-uncased", num_labels=7)
out = m(**tok("Barack Obama lives in New York.", return_tensors="pt"))
print(out.logits.shape)  # (1, tokens, 7) per-token NER scores
```
...
m = AutoModelForQuestionAnswering.from_pretrained("distilbert-base-uncased-distilled-squad")
out = m(**tok("Who?", "BERT was made by Google.", return_tensors="pt"))
print(out.start_logits.shape, out.end_logits.shape)  # span start/end dists
```
...
weights = torch.tensor([1.0, 9.0])  # minority class gets higher weight
loss = torch.nn.CrossEntropyLoss(weight=weights)
print(loss(m(input_ids=torch.tensor([[101, 2023, 102]])).logits, torch.tensor([0])))
```
...
args = TrainingArguments("out", learning_rate=2e-5,
                         warmup_steps=500, weight_decay=0.01)
print(args.learning_rate)  # 2e-5..5e-5 is typical for BERT fine-tuning
```
...
opt = AdamW(m.parameters(), lr=2e-5, weight_decay=0.01)  # decoupled weight decay
print(opt.defaults["weight_decay"])
```
...
print(args.num_train_epochs)  # 2-4 epochs, early stop on validation
```
...
```python
from transformers import TrainingArguments
args = TrainingArguments("out", per_device_train_batch_size=8,
                         gradient_accumulation_steps=4)
print(args.per_device_train_batch_size * args.gradient_accumulation_steps)  # 32 effective
```
...
```python
from transformers import TrainingArguments
args = TrainingArguments("out", learning_rate=1e-5, weight_decay=0.01,
                         num_train_epochs=3, load_best_model_at_end=True)
print(args.weight_decay)  # dropout(0.1) is already inside BERT models
```
...
base_lr = 5e-5
param_groups = []
for i, layer in enumerate(model.bert.encoder.layer):  # highest = top layer
    lr = base_lr * (0.95 ** (11 - i))
    param_groups.append({"params": layer.parameters(), "lr": lr})
opt = torch.optim.AdamW(param_groups, lr=base_lr)
print("layer12 lr:", round(base_lr, 7), "layer1 lr:", round(base_lr * 0.95 ** 11, 7))
```
...
bert = AutoModelForMaskedLM.from_pretrained("bert-base-uncased")  # encoder, MLM
gpt = AutoModelForCausalLM.from_pretrained("gpt2")                # decoder, causal
print(type(bert).__name__, type(gpt).__name__)
```
...
t5 = AutoModelForSeq2SeqLM.from_pretrained("t5-small")           # encoder-decoder
print(type(t5).__name__)  # text-to-text: everything is (input text -> output text)
```
...
```python
from transformers import AutoModelForSequenceClassification
xl = AutoModelForSequenceClassification.from_pretrained("xlnet-base-cased")  # permutation LM
print(type(xl).__name__)
```
...
print(sum(p.numel() for p in teacher.parameters()))  # teacher is much larger
```
...
print("quantized to int8, ~4x smaller")
```
...
dummy = torch.randint(0, 30522, (1, 32))
torch.onnx.export(m, (dummy, torch.ones(1, 32, dtype=torch.long),
                       torch.zeros(1, 32, dtype=torch.long)),
                  "bert.onnx", opset_version=14,
                  input_names=["input_ids", "attention_mask", "token_type_ids"])
print("exported bert.onnx")
```
...
**Code:**
```python
m.eval()  # quantization + distillation + ONNX Runtime + batching
import onnxruntime as ort
sess = ort.InferenceSession("bert.onnx", providers=["CUDAExecutionProvider", "CPUExecutionProvider"])
...
out = sess.run(None, {"input_ids": np.ones((1, 32), dtype=np.int64),
                      "attention_mask": np.ones((1, 32), dtype=np.int64),
                      "token_type_ids": np.zeros((1, 32), dtype=np.int64)})
print(type(out[0]), out[0].shape)  # optimized inference result
```
...
def sentiment(text: str):
    return clf(text)  # served via Docker/FastAPI in production
```
...
ds = load_dataset("imdb", split="train[:100]").map(
    lambda x: tok(x["text"], truncation=True, padding=True), batched=True)
ds = ds.rename_column("label", "labels").select_columns(["input_ids", "attention_mask", "labels"])
...
trainer.train()  # handles loops, checkpointing, logging, distributed
```
...
```python
from transformers import TrainingArguments
args = TrainingArguments("out", fp16=True)   # automatic mixed precision (AMP)
print(args.fp16)
```
...
print(deepspeed_cfg["zero_optimization"]["stage"])
```
...
**Code:**
```python
zero_cfg = {"zero_optimization": {"stage": 3, "partition_activations": True},
              "gradient_accumulation_steps": 8}
# stage 3 shards optimizer states, gradients AND parameters across GPUs
print(zero_cfg["zero_optimization"]["stage"])
```
...
print(analyzer.polarity_scores("VADER is awesome!"))
```
...
```python
from nltk.sentiment.vader import SentimentIntensityAnalyzer
ka = SentimentIntensityAnalyzer()
print(ka.lexicon.get("great"))          # valence: -4 .. +4
print(ka.polarity_scores("very good"))   # booster word "very"
print(ka.polarity_scores("not good"))    # negation flips/boosts
```
...
```python
from nltk.sentiment.vader import SentimentIntensityAnalyzer
s = SentimentIntensityAnalyzer().polarity_scores("I love this place!")
print(s)          # {'neg':.., 'neu':.., 'pos':.., 'compound':..}
print(round(s["pos"] + s["neu"] + s["neg"], 2))  # ~= 1.0
```
...
from nltk.sentiment.vader import SentimentIntensityAnalyzer
analyzer = SentimentIntensityAnalyzer()
scores = analyzer.polarity_scores("The movie was not bad at all.")
print(scores)  # dict with neg, neu, pos, compound
```
...
print(a.polarity_scores("This is AMAZING!!!"))       # CAPS + punctuation
print(a.polarity_scores("OMG you're the best :) "))  # slang + emoticon
```
...
# 'great' reads positive even though the sentence is sarcastic/negative
```
...
from nltk.sentiment.vader import SentimentIntensityAnalyzer
a = SentimentIntensityAnalyzer()
print(a.polarity_scores("The food is good")["compound"])
print(a.polarity_scores("The food is not good")["compound"])  # flipped
```
...
from nltk.sentiment.vader import SentimentIntensityAnalyzer
a = SentimentIntensityAnalyzer()
for t in ["good", "very good", "extremely good", "somewhat good"]:
...
# clause after 'but' outweighs the clause before it
```
...
from nltk.sentiment.vader import SentimentIntensityAnalyzer
a = SentimentIntensityAnalyzer()
print(a.polarity_scores("The service was GREAT")["compound"])
print(a.polarity_scores("The service was great")["compound"])  # less intense
```
...
from nltk.sentiment.vader import SentimentIntensityAnalyzer
a = SentimentIntensityAnalyzer()
print(a.polarity_scores("What a beautiful day :) "))
print(a.polarity_scores("What a beautiful day :( "))
print(a.lexicon.get(":)")) 
```
...
```python
from nltk.sentiment.vader import SentimentIntensityAnalyzer
s = SentimentIntensityAnalyzer().polarity_scores("This is good but boring.")
print(s["compound"])  # a single -1..+1 figure summarizing pos/neg/neu
```
...
```python
import math
def normalize(score_sum, alpha=15):
    return score_sum / math.sqrt(score_sum ** 2 + alpha)
print(normalize(3.0))    # small text, dampened
print(normalize(10.0))   # longer text stays in [-1, 1]
```
...
def label(compound):
    ...
    return "neutral"
a = SentimentIntensityAnalyzer()
for text in ["superb", "meh", "dreadful"]:
    print(text, label(a.polarity_scores(text)["compound"]))
```
...
from nltk.sentiment.vader import SentimentIntensityAnalyzer
a = SentimentIntensityAnalyzer()
a.lexicon.update({"bullish": 2.5, "downgrade": -2.0})  # domain words
print(a.polarity_scores("The stock looks bullish"))
print(a.polarity_scores("The rating was a downgrade"))
```
...
print(TextBlob(text).sentiment)  # polarity + subjectivity, simpler lexicon
```
...
from nltk.sentiment.vader import SentimentIntensityAnalyzer
from transformers import pipeline
text = "The acting was brilliant even though the plot dragged."
print(SentimentIntensityAnalyzer().polarity_scores(text)["compound"])
print(pipeline("sentiment-analysis")(text))
```
...
from nltk.sentiment.vader import SentimentIntensityAnalyzer
a = SentimentIntensityAnalyzer()
# needs speed, no labels, social-media style text, explainability -> VADER
def choose(speed, no_labels, social):
    return "VADER" if speed and no_labels and social else "BERT"
print(choose(speed=True, no_labels=True, social=True))
```
...
from nltk.sentiment.vader import SentimentIntensityAnalyzer
a = SentimentIntensityAnalyzer()
tweets = ["Worst night of my life.", "Absolutely loved it!!"]
for t in tweets:
    print(t, a.polarity_scores(t)["compound"])  # fast but shallow
```
...
v = vader(text)["compound"] / 2 + 0.5           # scale to 0..1
b = float(bert(text)[0]["score"])               # BERT probability
ensemble = 0.5 * v + 0.5 * b
print(round(ensemble, 3))
```
...
tok = AutoTokenizer.from_pretrained("bert-base-uncased")
m = AutoModelForSequenceClassification.from_pretrained("bert-base-uncased", num_labels=3)
# aspect + review -> sentiment for that aspect
for aspect in ["food", "service"]:
    out = m(**tok(aspect, "The food was great but the service was slow.",
                  return_tensors="pt"))
    print(aspect, out.logits.argmax().item())
```
...
tok = AutoTokenizer.from_pretrained("bert-base-uncased")
m = AutoModelForSequenceClassification.from_pretrained("bert-base-uncased", num_labels=2)
train_ds = [{"input_ids": tok(t, truncation=True, padding=True)["input_ids"],
             "attention_mask": tok(t, truncation=True, padding=True)["attention_mask"],
             "labels": l} for t, l in [("I love this.", 1), ("Terrible.", 0)]]
...
print(clf("This movie was a masterpiece."))
```
...
# sarcasm needs contextual training; BERT learns cues from labeled data
m = AutoModelForSequenceClassification.from_pretrained("bert-base-uncased", num_labels=2)
tok = AutoTokenizer.from_pretrained("bert-base-uncased")
print(m(**tok("Oh great, it's raining again.", return_tensors="pt")).logits)
```
...
tok = AutoTokenizer.from_pretrained("bert-base-uncased")
m = AutoModelForSequenceClassification.from_pretrained("bert-base-uncased", num_labels=2)
out = m(**tok("The service was NOT good.", return_tensors="pt"))
print(out.logits)  # attention links "NOT" with "good" directly
```
...
a = SentimentIntensityAnalyzer()
tok = BertTokenizer.from_pretrained("bert-base-uncased")
raw, cleaned = "OMG!!! :D ", "omg"
print(a.polarity_scores(raw)["compound"])     # punctuation/caps preserved
print(a.polarity_scores(cleaned)["compound"])  # aggressive cleaning kills signal
print(tok("The movie was great.", return_tensors="pt")["input_ids"])
```
...
from transformers import AutoModelForSequenceClassification
m = AutoModelForSequenceClassification.from_pretrained("bert-base-uncased", num_labels=2)
print(sum(p.numel() for p in m.parameters()))  # 110M params to serve
```
...
from nltk.sentiment.vader import SentimentIntensityAnalyzer
a = SentimentIntensityAnalyzer()
print(a.polarity_scores("This award-winning film really tied the room together.")["compound"])
# no understanding of narrative context/sarcasm
```
...
```python
from transformers import AutoModelForSequenceClassification
binary = AutoModelForSequenceClassification.from_pretrained("bert-base-uncased", num_labels=2)
multi  = AutoModelForSequenceClassification.from_pretrained("bert-base-uncased", num_labels=3)
fine   = AutoModelForSequenceClassification.from_pretrained("bert-base-uncased", num_labels=5)
print(binary.num_labels, multi.num_labels, fine.num_labels)
```
...
probs = torch.sigmoid(logits)  # each label independent in [0, 1]
print(probs)
```
...
for s in ["This is great", "Esto es genial", "C'est super"]:
    print(s, m(**tok(s, return_tensors="pt")).logits.shape)
```
...
out = m(**tok("Dieses Produkt ist großartig", return_tensors="pt"))
...
```python
from transformers import AutoTokenizer
unc = AutoTokenizer.from_pretrained("bert-base-uncased")
cas = AutoTokenizer.from_pretrained("bert-base-cased")
print(unc.tokenize("Apple"))  # ['apple'] lowercased
print(cas.tokenize("Apple"))  # ['Apple'] case kept
```
...
print(aug.augment("The movie was really great"))  # synonym replacement
```
...
```python
import nlpaug.augmenter.word as naw
bt = naw.BackTranslationAug(aug_src="en", aug_pool_size=5)
print(bt.augment("The product exceeded my expectations"))
```
...
m = AutoModelForMaskedLM.from_pretrained("bert-base-uncased")   # DAPT
tok = AutoTokenizer.from_pretrained("bert-base-uncased")
collator = DataCollatorForLanguageModeling(tok, mlm_probability=0.15)
print(type(collator).__name__, m.config.model_type)
```
...
print(finbert("Company beat earnings expectations by 20%."))
```
...
m = AutoModelForTokenClassification.from_pretrained("dmis-lab/biobert-base-cased-v1.1")
tok = AutoTokenizer.from_pretrained("dmis-lab/biobert-base-cased-v1.1")
print(m(**tok("EGFR mutation was detected in the lung", return_tensors="pt")).logits.shape)
```
...
from nltk.sentiment.vader import SentimentIntensityAnalyzer
a = SentimentIntensityAnalyzer()
stream = ["market crash imminent", "great gains today", "just another tweet"]
for tweet in stream:                       # e.g. Kafka/Flink consumer
    print(tweet, a.polarity_scores(tweet)["compound"])   # microsecond latency
```
...
# audit accuracy separately per demographic slice to surface bias
groups = {"men":    ([1, 0, 1, 0], [1, 1, 1, 0]),
          "women":  ([0, 1, 0, 1], [0, 1, 0, 1]),
          "non-native": ([0, 0, 1, 1], [1, 0, 1, 0])}
for group, (y_t, y_p) in groups.items():
    print(group, round(accuracy_score(y_t, y_p), 3))
```
...
out = m(**tok("The movie was amazing but slow.", return_tensors="pt"))
print(out.attentions[0].shape)  # per-layer attention, for visualization
```
...
def production_sentiment(text):
    c = vader.polarity_scores(text)["compound"]
    if abs(c) > 0.8:                      # VADER fast path, most traffic
        return "vader", c
    r = bert(text)[0]                     # BERT slow path for difficult cases
    return "bert", r["label"], r["score"]

print(production_sentiment("Best movie of the year!!"))
