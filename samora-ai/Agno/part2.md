...
agent = Agent(
    memory=VectorMemory(vector_db=ChromaDb(), session_id="user_123"),
    enable_memory=True,
...
coordinator = Agent(name="Coordinator", description="Splits work and synthesizes results.")
researcher = Agent(name="Researcher", description="Finds facts.", instructions=["Return sources."])
writer = Agent(name="Writer", description="Writes final copy.")
reviewer = Agent(name="Reviewer", description="Checks quality.")
...
team.print_response("Research and write a 3-paragraph report on renewable energy.")
```
...
class InternalLlmChat(Model):
    def invoke(self, messages: list[Message], **kwargs) -> Message:
        prompt = "\n".join(m.content for m in messages if m.content)
        return Message(content=self.internal_chat(prompt), role="assistant")

    async def invoke_stream(self, messages: list[Message], **kwargs):
...
agent = Agent(model=InternalLlmChat())
agent.print_response("Hello from the internal API!")
```
...
agent = Agent(
    description="Keeps only the most recent context.",
    add_history_to_messages=True,
    max_history_messages=20,
)
agent.print_response("Let us keep this conversation short and focused.")
```
...
tool_suite = {
    "weather": Agent(tools=[get_weather], description="Weather specialist."),
    "finance": Agent(tools=[get_stock_price], description="Finance specialist."),
}

def route_task(task: str) -> str:
    if "weather" in task.lower():
        return tool_suite["weather"].run(task).content
    return tool_suite["finance"].run(task).content

router = Agent(tools=[route_task], show_tool_calls=True)
router.print_response("Is it sunny today and what is AAPL at?")
```
...
def lookup_pricing(product: str) -> str:
    ...
    return {"laptop": "$999", "monitor": "$299"}.get(product, "Not found")

def check_stock(item: str) -> str:
    """Return stock availability for an item."""
    return {"laptop": "In stock", "monitor": "Out of stock"}.get(item, "Unknown")

agent = Agent(tools=[lookup_pricing, check_stock], show_tool_calls=True)
agent.print_response("Get price and stock for the laptop in one turn.")
```
...
kb = KnowledgeBase()
doc = Document(name="guide-v2", content="Updated instructions.", metadata={"version": 2})
kb.add_documents([doc])
prior = [d for d in kb.documents if d.metadata.get("version", 0) < 3]
print(len(prior))
```
...
def step_tool(name: str) -> str:
    """Record an instrumentation event."""
    return f"span: {name} traced"
...
agent.print_response("Run one instrumented task.")
```
...
def score_response(candidate: str) -> str:
    """Score a candidate prompt on word reuse."""
    tokens = set(candidate.lower().split())
    return f"{len(tokens)} unique words"

optimizer = Agent(
    tools=[score_response],
    description="Tests prompt variants and keeps the best.",
)
optimizer.print_response("Optimize: 'make it shorter and clearer'")
```
...
class Report:
    def __init__(self, caption: str, table_count: int):
        self.caption = caption
        self.table_count = table_count

def index_page(path: str) -> Report:
    """Extract text, images, and table counts from a page."""
    return Report(caption=f"Parsed {path}", table_count=2)

agent = Agent(tools=[index_page])
result = agent.run(
    "Summarize this earnings page and list its tables.",
    images=[Image(filepath="/tmp/report.png")],
)
...
def answer(prompt: str) -> str:
    """Return a synthetic cached answer."""
...
agent = Agent(tools=[cached_answer])
print(cached_answer("What is 2+2?"))
print(cached_answer.cache_info())
```
...
def process_messages() -> str:
    """Return a lazy handle instead of doing expensive work."""
    return "PendingJobsFuture"

def drain(payload: str) -> str:
    """Execute the deferred work now."""
    return f"Executed: {payload}"

agent = Agent(tools=[process_messages, drain], show_tool_calls=True)
agent.print_response("Prepare the job list, then execute lazily when asked.")
```
...
def detect_language(text: str) -> str:
    """Return the ISO code of the input language."""
    return "en"
...
agent = Agent(tools=[detect_language, translate], show_tool_calls=True)
agent.print_response("Hello, welcome. Respond in the same language.")
```
...
class DailyDigestWorkflow(Workflow):
    def _run(self, topic: str) -> RunResponse:
        return RunResponse(content=f"Digest for {topic}")

def on_schedule(*_):
    workflow = DailyDigestWorkflow()
    result = workflow.run(topic="agno releases")
    print(result.content)

from apscheduler.schedulers.blocking import BlockingScheduler

scheduler = BlockingScheduler()
scheduler.add_job(on_schedule, "cron", hour=9, minute=0)
scheduler.start()
```
...
def get_weather_batch(cities: list[str]) -> str:
    """Return weather for many cities in one call."""
    return "\n".join(f"{c}: sunny" for c in cities)

agent = Agent(tools=[get_weather_batch], show_tool_calls=True)
agent.print_response("Weather for Oslo, Tokyo, and Lisbon at once.")
```
...
def call_api(endpoint: str) -> str:
    """Call an endpoint with the current key, rotating on error."""
    return "200 OK"

manager = CredentialManager(keys=["key-1", "key-2"])
agent = Agent(
    tools=[call_api],
    instructions=[
        f"dbt_api_key is '{manager.rotate()}'"
    ],
)
agent.print_response("Call the reports endpoint and show the status.")
```
...
def limited_call(query: str) -> str:
    """Return result or a retry-later signal."""
    return "OK" if bucket.consume() else "rate limited; retry later"

agent = Agent(tools=[limited_call])
agent.print_response("Call the API three times and report results.")
```
...
def search_facts(query: str) -> str:
    """Return verified facts from an internal source."""
    return {"capital of France": "Paris"}.get(query.lower(), "no source found")

def cross_check(claim: str) -> str:
    """Confirm a claim has a supporting source."""
    hits = search_facts(claim)
    return "verified" if "Paris" in hits else "unsupported"

agent = Agent(tools=[cross_check], instructions=["Flag claims that cross_check marks unsupported."])
agent.print_response("Is Paris the capital of France?")
```
...
TOOL_REGISTRY = {"get_weather", "get_stock_price", "send_email"}

def list_tools() -> str:
    """List every registered tool name."""
    return ", ".join(sorted(TOOL_REGISTRY))

def get_tool_schema(name: str) -> str:
    """Return the parameter schema for a named tool."""
    return f"{{'name': '{name}', 'parameters': ['query']}}"

agent = Agent(tools=[list_tools, get_tool_schema])
agent.print_response("Discover available tools and describe get_weather.")
```
...
mistake_memory = VectorMemory(vector_db=ChromaDb(), session_id="lessons")

def log_mistake(lesson: str) -> str:
    """Store a lesson learned to recall later."""
    return f"remembered: {lesson}"

agent = Agent(
    memory=mistake_memory,
    tools=[log_mistake],
    enable_memory=True,
)
agent.print_response("Remember that division by zero raises an error.")
agent.print_response("What lesson did I learn about division by zero?")
```
...
agent = Agent(
    description="Summarizes long conversations to a fixed budget.",
    add_history_to_messages=True,
    max_history_messages=6,
)
agent.print_response("Summarize this chat so far in one short paragraph.")
```
...
class ResearchThenSummarize(Workflow):
    def _run(self, topic: str) -> RunResponse:
        notes = f"notes about {topic}"
        summary = f"summary of {notes}"
        return RunResponse(content=summary)

workflow = ResearchThenSummarize()
...
def get_weather(city: str) -> str:
    """Return the weather for a city only."""
    return f"{city}: sunny"

tool_agent = Agent(tools=[get_weather], show_tool_calls=True, tool_choice="required")
tool_agent.print_response("Tell me the weather in Lisbon.")

plain_agent = Agent(tools=[get_weather], show_tool_calls=True, tool_choice="auto")
plain_agent.print_response("Hello, no tools needed here.")
```
...
kb = PDFKnowledgeBase(path="live/")
kb.load()
agent = Agent(knowledge=kb, search_knowledge=True, add_references=True)
agent.print_response("What is the latest policy change?")
```
...
def run_for_tenant(session_id: str) -> Agent:
    return Agent(
        description="Tenant isolated agent.",
        session_id=f"tenant_123:{session_id}",
        add_history_to_messages=True,
    )

agent = run_for_tenant("session_456")
agent.print_response("Only my tenant's history should be visible.")
```
...
class GapAwareKnowledgeBase(KnowledgeBase):
    def __init__(self):
        super().__init__()
        self.gaps = []

    def report_gap(self, query: str, missing: str) -> None:
        self.gaps.append({"query": query, "missing_terms": missing})

kb = GapAwareKnowledgeBase()
kb.report_gap("How does pricing work?", "regional_pricing")
print(kb.gaps)
```
...
def heavy_task(query: str) -> str:
    """A long-running tool with visible progress signals."""
    return "finished heavy task"

agent = Agent(tools=[heavy_task], show_tool_calls=True)
for event in agent.run_stream("Run the heavy task"):
    if event.event_type == "ToolCallStarted":
        print(f"[status] calling {event.content}")
...
def is_topical(topic: str) -> str:
    """Return whether a topic is relevant this session."""
    return "relevant" if topic.lower() in {"agno", "agents"} else "irrelevant"

context_aware = Agent(tools=[is_topical], show_tool_calls=True)
context_aware.print_response("Should I suggest the weather tool now?")
```
...
classifier = Agent(name="Router", description="Classifies task type.")
coder = Agent(name="Coder", description="Writes code.", instructions=["Return code only."])
writer = Agent(name="Writer", description="Writes prose.", instructions=["Return prose only."])

team = Team(
    name="Hybrid Team",
    leader=classifier,
    members=[coder, writer],
    mode="route",
)
team.print_response("Write a python function to reverse a string.")
```
...
shared_pool = VectorMemory(vector_db=ChromaDb(), session_id="shared")

def share_note(text: str) -> str:
    """Write a note to the shared memory pool."""
    return f"stored: {text}"

worker_a = Agent(memory=shared_pool, tools=[share_note], enable_memory=True)
worker_b = Agent(memory=shared_pool, enable_memory=True)
worker_a.print_response("Store the fact that the vault key is in Oslo.")
worker_b.print_response("Where is the vault key?")
```
...
agent = Agent(
    ...
    instructions=[
        "After each tool call, explain why you chose it and what it returns."
    ],
)
agent.print_response("Convert 100 EUR to USD and explain your steps.")
```
...
def query_kb(query: str, chunk_size: int = 512) -> str:
    """Retrieve with an adaptive chunk size."""
    return f"retrieved {chunk_size} with {query}"

agent = Agent(tools=[query_kb], show_tool_calls=True)
agent.print_response("Retrieve details about 'product refund policy' with a small chunk.")
```
...
def discover_schema() -> str:
    ...
    return f"tables={list(SCHEMA)}"

def run_sql(query: str) -> str:
    """Execute a SQL statement and return rows."""
    return "row"

agent = Agent(tools=[discover_schema, run_sql], show_tool_calls=True)
agent.print_response("What tables exist, then count orders per user.")
```
...
full_history = VectorMemory(vector_db=ChromaDb(), session_id="day-1")

def recall_topics() -> str:
    """Retrieve the full, uncompressed conversation context."""
    return "full context: we discussed pricing and refunds"
...
agent.print_response("Expand the summary back into the full discussion.")
```
...
def classify_intent(text: str) -> str:
    """Return intent category for a user request."""
    lowered = text.lower()
    if "password" in lowered or "ssn" in lowered:
        return "sensitive"
    if text.endswith("?"):
        return "question"
    return "command"
...
agent = Agent(
    ...
    instructions=[
        "Block sensitive requests, answer questions, and only run commands when safe."
    ],
)
agent.print_response("What is my SSN?")
```
...
def record_feedback(response_id: str, rating: int) -> str:
    """Store a rating for a past response."""
    FEEDBACK.append({"response_id": response_id, "rating": rating})
    return "thanks"

agent = Agent(tools=[record_feedback], show_tool_calls=True)
agent.print_response("Mark the last answer as helpful with rating 5.")
```
...
ACCESS_LABELS = {"public": {"logo.pdf"}, "internal": {"roadmap.pdf"}, "confidential": {"comp.ipynb"}}

def retrieve_docs(query: str, clearance: str = "public") -> str:
    """Retrieve only documents permitted for a clearance level."""
    allowed = set()
    for level in ("public", "internal", "confidential"):
        if clearance == level or (clearance == "internal" and level == "public"):
            allowed |= ACCESS_LABELS[level]
    return ", ".join(sorted(allowed))

agent = Agent(tools=[retrieve_docs])
agent.print_response("Which documents can I see?")
```
...
PROFILE = {"name": "Ana", "plan": "pro", "region": "EU"}

def build_system_prompt(profile: dict) -> str:
    return (
        f"You are a concierge for {profile['name']} on the {profile['plan']} plan "
        f"serving the {profile['region']} region."
    )

agent = Agent(
    description=build_system_prompt(PROFILE),
    instructions=["Be concise."],
)
agent.print_response("Suggest a plan change for me.")
```
...
def respond_in_kind(text: str) -> str:
    """Reply mirroring the detected language."""
    lang = detect_language(text)
    return {"es": "Hola, ¿cómo estás?", "en": "Hello, how are you?"}.get(lang, "Hello")

agent = Agent(tools=[respond_in_kind], show_tool_calls=True)
agent.print_response("Hello there!")
```
...
TOOL_STATS = {"get_weather": {"calls": 0, "errors": 0, "latency": []}}

def timed(fn_name: str, fn):
    def wrapper(*args, **kwargs):
        start = time.monotonic()
        try:
            return fn(*args, **kwargs)
        except Exception:
            TOOL_STATS[fn_name]["errors"] += 1
            raise
        finally:
            TOOL_STATS[fn_name]["latency"].append(time.monotonic() - start)
            TOOL_STATS[fn_name]["calls"] += 1
    return wrapper
...
get_weather = timed("get_weather", get_weather)
agent = Agent(tools=[get_weather], show_tool_calls=True)
agent.print_response("Weather in Rome.")
print(TOOL_STATS)
```
...
VERSIONS = {"v1": ["doc-1"], "v2": ["doc-1", "doc-2"]}

class VersionedKnowledgeBase(KnowledgeBase):
    def __init__(self):
        super().__init__()
        self.version = "v2"
    ...
    def upsert(self, doc_id: str) -> None:
        VERSIONS["v3"] = VERSIONS[self.version] + [doc_id]
        self.version = "v3"

kb = VersionedKnowledgeBase()
kb.upsert("doc-3")
print(kb.snapshot(), VERSIONS[kb.snapshot()])
```
...
def hybrid_search(query: str) -> str:
    """Vector search with a BM25 fallback."""
    if VECTOR_DB_UP:
        return "vector results"
    return "bm25 results"

def answer_from_memory(query: str) -> str:
    """Fall back to parametric knowledge when no store is reachable."""
    return "I can answer from general knowledge."

agent = Agent(
    tools=[hybrid_search, answer_from_memory],
    instructions=["Prefer retrieved facts; state it when answering from general knowledge."],
)
agent.print_response("What is the capital of Norway?")
```
...
def watcher(prompt: str) -> Agent:
    words = len(prompt.split())
    model_id = "gpt-4o" if words > 200 else "gpt-4o-mini"
    return Agent(
...
agent = Agent(
    ...
    instructions=["Encrypt conversation contents before storing."],
)
agent.print_response("Save my session note safely.")
```
...
agent = Agent(
    ...
    instructions=["If the answer is not in context, retrieve more documents before replying."],
)
agent.print_response("What is the refund policy?")
```
...
def verify_citation(claim: str, source: str) -> str:
    """Check whether a source actually supports a claim."""
    return "supported" if "Paris" in source else "unsupported"

def cite(claim: str) -> str:
    """Return the citation found for a claim."""
    return "Capital of France -> wiki: Paris is the capital."

agent = Agent(
    tools=[verify_citation, cite],
    instructions=["Drop or flag citations that verify_citation rejects."],
)
agent.print_response("State the capital of France with a citation and verify it.")
```
...
persona_memory = VectorMemory(vector_db=ChromaDb(), session_id="ana-persona")

PERSONA = {"name": "ana", "traits": "friendly expert", "style": "concise"}

def load_persona() -> str:
    """Load persona traits for this session."""
    return f"You are a {PERSONA['traits']}, style: {PERSONA['style']}"

agent = Agent(
    description=load_persona(),
    memory=persona_memory,
    enable_memory=True,
)
agent.print_response("Who are you and what do you remember about me?")
```
...
def check_quota() -> str:
    """Return remaining daily calls for the current account."""
    return f"{DAILY_LIMIT - USAGE[ACCOUNT]} calls left"

def track_call() -> str:
    """Consume one unit of the daily quota."""
    if USAGE[ACCOUNT] >= DAILY_LIMIT:
        return "quota exceeded"
    USAGE[ACCOUNT] += 1
    return f"used {USAGE[ACCOUNT]}/{DAILY_LIMIT}"

agent = Agent(tools=[check_quota, track_call], show_tool_calls=True)
agent.print_response("Run my daily task and show remaining quota.")
```
